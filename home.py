...
with tab1:
    ...
    st.markdown("Go Cattle is a :green[**Cattle Healthcare Platform**]. India is the Home to about 17% of the World's Cows and For Every 1 Registered Vet in India, There are about 50,000 cows. Due To These Reasons, About 70% of The Cows Cannot Get Proper Healthcare and Treatments. It is Very Important to increase awareness about this topic because This Leads to Thousands if Not Hundreds of Thousands of Cattle Dying Every Year.")
...
with tab2:
    ...
    if selected2 == "Technicalities":
        ...
        st.markdown("""
                ...
                """)
        st.markdown("Our ML-model boasts an outstanding :green[**accuracy rate of 68%+**], surpassing the required medical standards" f"<sup>#</sup>" " Developed using a vast dataset of *20,499 parameters* sourced from reliable and up-to-date information gathered through web crawling & web scraping, Go Cattle provides a robust foundation for precise disease identification.", unsafe_allow_html=True)
        st.markdown("Equipped with an extensive range of 123 unique symptoms and a comprehensive list of 163 unique diseases, Go Cattle covers a wide spectrum of ailments that can affect cattle. By inputting the observed symptoms, the system swiftly processes the information and generates a reliable diagnosis, enabling prompt action to be taken. :violet[ *The Dataset has been gone through Vigorous Changes Recently and There's A High Possibility that our team might have messed up something in the Process*]")
...
In this case, the cross-validation is performed using two folds. The mean accuracy of the model is 68%.
...
The accuracy of the model is then evaluated by comparing the predicted values to the actual values. The accuracy in this case is 67.5%.
...
The accuracy of the model is then evaluated by comparing the predicted values to the actual values. The accuracy in this case is 67%.
Overall, the random forest classification model achieves an accuracy of 67.5% on the test set. This means that the model is able to correctly predict the disease for 85% of the test data.
                    """)
...
with tab3:
    st.title("Credits")
    st.markdown("""
As Indians, we’ve had early experiences with cows in our villages. We’ve seen firsthand how important it is to keep cows healthy, and we’ve also seen the challenges that farmers face in providing quality healthcare to their animals.

We also got the inspiration for this idea from the Nandini project. We were really impressed by the idea and wanted to make something similar but with a more modern approach. Initially, we just wanted to see how fast it would take us to collect the data and make a model, but we ended up making a full-fledged healthcare platform.

We really hope that this project helps the farmers and the cattle in some way or another. Thanks to all the people that helped along the way and contributed towards this project. We really hope that machine learning and artificial intelligence contribute toward society and nature in a beneficial way like this. (We’re looking at you, Cambridge Analytica.)

We hope to make projects like [:green[Go-Cattle]](#) and [:orange[Rellekk-Z]](https://rellekk-z.github.io/Rellekk-Z)
and [:orange[Decodificate]](https://decodificate.tech) in the future that aim to seamlessly blend in with the cause of social benefit, and we hope that you’ll be there to support us. Thank you for using Go Cattle.""")
